    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      <div class="p-10">
        ...
        <div v-if="step === 'search'">
          <h2 class="text-2xl font-bold text-gray-900 mb-2">Buscar Perfil</h2>
          <p class="text-gray-500 text-sm mb-8">Insira o @ do usuário ou o link do perfil do Instagram.</p>

          <div class="flex gap-4">
            <input 
              v-model="searchQuery" 
              type="text" 
              class="flex-1 px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none transition-all shadow-sm"
        ...
        <div v-if="step === 'confirm'" class="max-w-2xl mx-auto">
          <div class="text-center mb-8">
            <h2 class="text-2xl font-bold text-gray-900">É esse perfil?</h2>
            <p class="text-gray-500 mt-2">Confirme se as informações abaixo correspondem ao alvo desejado.</p>
          </div>

          <!-- Card do Perfil -->
          <div class="bg-gray-50 border border-gray-200 rounded-2xl p-8 mb-8 text-center shadow-sm">
            ...
            <h3 class="text-xl font-bold text-gray-900">{{ profileData.nome }}</h3>
            <p class="text-pink-600 font-medium text-sm mb-6">@{{ profileData.username }}</p>

            <div class="flex justify-center gap-8 mb-6 border-y border-gray-200 py-4">
              <div>
                <p class="text-lg font-bold text-gray-900">{{ profileData.seguidores }}</p>
                <p class="text-xs text-gray-500 uppercase tracking-wide">Seguidores</p>
              </div>
              <div class="w-px bg-gray-200"></div>
              <div>
                <p class="text-lg font-bold text-gray-900">{{ profileData.seguindo }}</p>
                <p class="text-xs text-gray-500 uppercase tracking-wide">Seguindo</p>
              </div>
              <div class="w-px bg-gray-200"></div>
              <div>
                <p class="text-lg font-bold text-gray-900">{{ profileData.posts }}</p>
                <p class="text-xs text-gray-500 uppercase tracking-wide">Posts</p>
              </div>
            </div>

            <div class="text-left bg-white p-4 rounded-xl border border-gray-100">
              <p class="text-gray-800 text-sm whitespace-pre-line mb-3">{{ profileData.bio }}</p>
          ...
          <div class="flex gap-4 justify-center">
            <button @click="cancelarBusca" class="px-8 py-3 rounded-xl border border-gray-300 text-gray-700 font-bold hover:bg-gray-50 transition-colors">
              Cancelar
            </button>
        ...
        <div v-if="step === 'config'">
          <div class="flex items-center gap-4 mb-8 border-b border-gray-100 pb-6">
            ...
            <div>
              <h2 class="text-xl font-bold text-gray-900">Configurar Extração</h2>
              <p class="text-gray-500 text-sm">Alvo: <span class="font-bold text-pink-600">@{{ profileData.username }}</span></p>
            </div>
          </div>

          <div class="max-w-xl">
            <div class="mb-6">
              <label class="block text-sm font-bold text-gray-700 mb-2">O que você deseja extrair?</label>
              <select v-model="extractType" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none shadow-sm">
            ...
            <div class="mb-8">
              <label class="block text-sm font-bold text-gray-700 mb-2">Quantidade Máxima de Leads</label>
              <input 
                type="number" 
                v-model="maxLeads"
                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none shadow-sm"
                placeholder="Ex: 500"
              />
              <p class="text-xs text-gray-500 mt-2">O Instagram limita o volume de dados. Recomendamos extrair até 2.000 por vez.</p>
            </div>

            <div class="flex gap-4">
              <button @click="step = 'search'" class="px-6 py-3 rounded-xl border border-gray-300 text-gray-700 font-bold hover:bg-gray-50 transition-colors">
                Voltar
              </button>
