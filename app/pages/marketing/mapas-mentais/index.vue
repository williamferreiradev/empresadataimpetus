  ...
  <div class="max-w-7xl mx-auto">
    <div class="flex justify-between items-center mb-8">
      <div>
        ...
        <p class="text-gray-500 mt-1">Gerencie e crie mapas mentais para o setor de marketing.</p>
      </div>
      <button 
        @click="criarNovoMapa" 
        :disabled="loading"
    ...
    <div v-else-if="mapas.length === 0" class="bg-white rounded-xl border border-gray-200 p-12 text-center shadow-sm">
      <div class="mx-auto w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center mb-4">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
        </svg>
      </div>
      <h3 class="text-lg font-medium text-gray-900 mb-2">Nenhum mapa mental</h3>
      <p class="text-gray-500 max-w-md mx-auto mb-6">Você ainda não criou nenhum mapa mental. Clique no botão acima para começar a organizar suas ideias.</p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div 
        ...
        class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm hover:shadow-md hover:border-orange-300 cursor-pointer transition-all group relative overflow-hidden"
      >
        ...
        <div class="flex justify-between items-start mb-4">
          ...
          <button @click.stop="deletarMapa(mapa.id)" class="text-gray-400 hover:text-red-500 transition-colors p-1" title="Excluir">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            </svg>
          </button>
        </div>
        <h3 class="text-lg font-semibold text-gray-900 mb-1 group-hover:text-orange-600 transition-colors">{{ mapa.nome }}</h3>
